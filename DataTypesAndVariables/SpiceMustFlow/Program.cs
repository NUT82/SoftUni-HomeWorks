...
namespace SpiceMustFlow
{
    class Program
    {
        static void Main(string[] args)
        {
            ...
            const int spiceConsumesByCrew = 26;
            int producedSpice = 0;
            int days = 0;
            //input
            ...
            while (sourceStartingYield >= 100)
            {
                producedSpice += sourceStartingYield - spiceConsumesByCrew; //The mining crew consumes 26 spice every day at the end of their shift
                days++;
                sourceStartingYield -= 10;
            }
            producedSpice -= spiceConsumesByCrew; //And additional 26 after the mine has been exhausted
            if (producedSpice < 0)
            {
                producedSpice = 0;
            }
            Console.WriteLine(days);
            Console.WriteLine(producedSpice);
        }
    }
}
